...
int32_t execute(const uint8_t* command)
{
    ...
    a =  get_current_terminal();
    tot_terminal.current_terminal = a;
    ...
    tot_terminal.terminal_pid_table[tot_terminal.current_terminal][tot_terminal.terminal_process_count[tot_terminal.current_terminal]] = index;
    tot_terminal.terminal_process_count[tot_terminal.current_terminal]++;
    tot_terminal.t_count ++;

    pcb_t * new_pcb =  (pcb_t*)(SIZE_OF_8MB - (index + 1) * SIZE_OF_8KB);
    if ((tot_terminal.terminal_process_count[tot_terminal.current_terminal] - 1) == 0){
        parent_pcb = NULL; // there is no process before
        new_pcb->parent_pid = index;
    }
    else{
        parent_pcb = (pcb_t*)(SIZE_OF_8MB -  (tot_terminal.terminal_pid_table[tot_terminal.current_terminal][tot_terminal.terminal_process_count[tot_terminal.current_terminal] -2] +1)* SIZE_OF_8KB);    // parent is the last process
    ...
    // int32_t next_pid = tot_terminal.terminal_pid_table[tot_terminal.current_terminal][tot_terminal.terminal_process_count[tot_terminal.current_terminal] - 1];
    // pcb_t *next_pcb = (pcb_t*)(0x800000 - 0x2000*(next_pid+1));
    // asm volatile(
    // "movl %%esp, %%eax;"
    // "movl %%ebp, %%ebx;"
...
int32_t getargs(uint8_t* buf, int32_t nbytes){
    ...
    asm("movl %%esp, %0" : "=r"(esp) :);
    pcb_t *pcb = (pcb_t *)( esp & PCB_MSK);   // get the pcb from the stack
    if(buf == NULL){
        return -1;
    }
    if((nbytes == 0)){
    ...
    for(i = 0; i < nbytes; i++){   // if it is valid, put it into the buffer
        buf[i] = pcb->args[i];
        pcb->args[i] = '\0';
    }
...
int32_t halt(uint8_t status){
    ...
    //tot_terminal.current_terminal = get_current_terminal();
    current_pcb_address = (pcb_t*)(SIZE_OF_8MB - SIZE_OF_8KB * (tot_terminal.terminal_pid_table[ter_scheduling][tot_terminal.terminal_process_count[ter_scheduling] - 1] + 1));      //find the pcb stack in the kernel page
    ...
    process_table[(unsigned int)(current_pcb_address->pid)] = 0;
    //update the terminal info
    tot_terminal.terminal_pid_table[ter_scheduling][tot_terminal.terminal_process_count[ter_scheduling]-1] = 0;
    tot_terminal.terminal_process_count[ter_scheduling]--;
    tot_terminal.t_count --;
    ...
    if (tot_terminal.t_count == 0 || tot_terminal.terminal_process_count[ter_scheduling] == 0){
        sti();
        execute((uint8_t*)"shell");                                                 //judge if shell is halt, if so, restart it
    }
    // if (tot_terminal.terminal_process_count[tot_terminal.current_terminal] == 0){
    //     if(tot_terminal.terminal_process_count[0] !=0){
    //         update_terminal(0);
    //     }
    //     else if (tot_terminal.terminal_process_count[1] != 0){
    //         update_terminal(1);
    //     }
    //     else if (tot_terminal.terminal_process_count[2] != 0)
    //     {
    //         update_terminal(2);
    //     }
    // }
...
/*
 *  Vidmap
 *  DESCRIPTION: map the video memory to the page
 *  INPUTS:             screen_start secondard pointer to the star of the screen                 
 *  OUTPUTS: NONE
 *  SIDEEFFECT: NONE
...
int32_t Vidmap(uint8_t** screen_start){
    int32_t tabel_index;
    if ((uint32_t)screen_start < SIZE_OF_128MB || (uint32_t)screen_start > SIZE_OF_128MB + SIZE_OF_4MB -4){
        return -1;      //check if the input is valid or not
    }
    *screen_start = (uint8_t*)VIDEO_ADDR;
    tabel_index = VIDEO_ADDR;
    tabel_index = tabel_index / SIZE_OF_4MB;        //calculate the table index
    PDE_TABLE[tabel_index].KBPDE.value = 0;
    PDE_TABLE[tabel_index].KBPDE.present = 1;
    PDE_TABLE[tabel_index].KBPDE.user_or_Supervisor = 1;
    PDE_TABLE[tabel_index].KBPDE.R_W = 1;
    PDE_TABLE[tabel_index].KBPDE.table_base_add = ((int32_t)&(PTE_TABLE_VID[0])) >> 12;     //pde table properties
    PTE_TABLE_VID[0].present = 1;
    PTE_TABLE_VID[0].R_W = 1;
    PTE_TABLE_VID[0].user_or_Supervisor = 1;    
    PTE_TABLE_VID[0].table_base_add = VIDEO_MEMORY>>12;         //pte table properties
